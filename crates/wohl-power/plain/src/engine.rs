...
impl PowerMonitor {
    ...
    pub fn process_reading(&mut self, circuit_id: u32, watts: u32, time: u64) -> PowerResult {
        let mut res = PowerResult {
            alerts: [PowerAlert::empty(); MAX_ALERTS_PER_READING],
            alert_count: 0,
        };

        // Phase 1: overconsumption via relay-lc (VERIFIED)
        let oc_result = self.watchpoints.evaluate(SensorReading {
            sensor_id: circuit_id,
            value: watts as i64,
        });
        if oc_result.violation_count > 0 && (res.alert_count as usize) < MAX_ALERTS_PER_READING {
            res.alerts[res.alert_count as usize] = PowerAlert {
                ...
                alert_type: PowerAlertType::OverConsumption,
                value: watts,
                threshold: oc_result.violations[0].threshold as u32,
                time,
            };
            res.alert_count += 1;
        }
        ...
        while i < count {
            ...
            if self.states[idx].active
                && self.configs[idx].circuit_id == circuit_id
                && self.configs[idx].enabled
            {
                if self.states[idx].last_time > 0 {
                    let last = self.states[idx].last_watts;
                    let diff = watts.abs_diff(last);
                    if diff > self.configs[idx].spike_threshold
    ...
    pub fn check_idle(&self, circuit_id: u32, current_watts: u32) -> bool {
        ...
        while i < self.circuit_count {
            let idx = i as usize;
            if self.configs[idx].circuit_id == circuit_id {
...
mod tests {
    ...
    fn test_empty() {
        ...
        let r = m.process_reading(1, 1000, 100);
        assert_eq!(r.alert_count, 0);
    }

    #[test]
    fn test_overconsumption_via_relay_lc() {
        let mut m = PowerMonitor::new();
        m.register_circuit(make_config(1));
    ...
    fn test_spike() {
        ...
        for j in 0..r.alert_count as usize {
            if r.alerts[j].alert_type == PowerAlertType::Spike {
                found = true;
            }
        }
        assert!(found);
    ...
    fn test_normal() {
        let mut m = PowerMonitor::new();
        m.register_circuit(make_config(1));
        assert_eq!(m.process_reading(1, 1500, 100).alert_count, 0);
    }
    #[test]
    fn test_unknown() {
        let mut m = PowerMonitor::new();
        assert_eq!(m.process_reading(99, 50000, 100).alert_count, 0);
    }
    #[test]
    fn test_idle() {
        let mut m = PowerMonitor::new();
        m.register_circuit(make_config(1));
        assert!(m.check_idle(1, 50));
        assert!(!m.check_idle(1, 200));
    }
}
