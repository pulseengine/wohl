...
mod tests {
    ...
    fn test_new_leak() {
        ...
        d.register_zone(1);
        assert_eq!(d.process_event(1, true, 1000), LeakAction::NewLeak);
    }
    #[test]
    fn test_already_wet() {
        let mut d = LeakDetector::new();
        d.register_zone(1);
        d.process_event(1, true, 1000);
        assert_eq!(d.process_event(1, true, 1001), LeakAction::AlreadyWet);
    }
    #[test]
    fn test_cleared() {
        let mut d = LeakDetector::new();
        d.register_zone(1);
        d.process_event(1, true, 1000);
        assert_eq!(d.process_event(1, false, 2000), LeakAction::Cleared);
    }
    #[test]
    ...
    fn test_unknown() {
        let mut d = LeakDetector::new();
        assert_eq!(d.process_event(99, true, 1000), LeakAction::Unknown);
    }
    #[test]
    fn test_multi_zone() {
        let mut d = LeakDetector::new();
        d.register_zone(1);
        d.register_zone(2);
        assert_eq!(d.process_event(2, true, 100), LeakAction::NewLeak);
        assert_eq!(d.process_event(2, false, 200), LeakAction::Cleared);
    }
    #[test]
    fn test_already_dry() {
        let mut d = LeakDetector::new();
        d.register_zone(1);
        assert_eq!(d.process_event(1, false, 1000), LeakAction::AlreadyDry);
    }
}
